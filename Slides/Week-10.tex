...
\begin{frame}{Sheet 7}
	...
	\uncover<3->{Thus, for $(x_0, y_0) = (0, 0),$ it is clear that it is a saddle point for $f$ as discriminant is $-4 < 0.$ }\\~\\
	\uncover<4->{Note that if $x = 0,$ the discriminant reduces to $-e^{-y^2}(y^6 - 3y^4 -8y^2 + 4).$ }\\
	\uncover<5->{Substituting $y = \pm\sqrt{2}$ gives us that the discriminant is positive and hence, the points are points of local minima. }\\~\\
	\uncover<6->{Similarly, we get that the points $(\pm\sqrt{2}, 0)$ are points of local maxima. }
\end{frame}
\begin{frame}{Sheet 7}
...
\begin{frame}{Sheet 7}
	...
	\uncover<2->{Note that the set $S^2 = \{(x, y, z) \in \mathbb{R}^3 : g(x, y, z) = 0\}$ is nonempty, closed and bounded, and $T$ is continuous on it. }\\
...
\begin{frame}{Sheet 7}
	...
	\uncover<4->{Thus, we get $yz = \lambda + \mu,\;zx = \lambda + \mu,\;xy = \lambda - \mu.$ \hfill $(*)$ }\\~\\
	\uncover<5->{Note that $g = h = 0$ gives us that $z = 20.$ }\\
	\uncover<6->{Also, the first two equalities of $(*)$ now give us that $x = y.$ }\\
	\uncover<7->{Combining that with $h = 0$ gives us $x = y = 10.$ }\\~\\
	\uncover<8->{Thus, we get $(x, y, z) = (10, 10, 20)$ as the only solution along with $(\lambda, \mu) = (150, 50).$ }\\
	\uncover<9->{Now, we calculate $f$ at the above point to get $2000$ as an extremal value. To see that it is indeed the maximum and not the minimum, we substitute some other point like $(0, 20, 20)$ satisfying the constraints and compare. }
\end{frame}
\begin{frame}{Sheet 7}
	...
	\uncover<6->{Now, the equalities of $(*)$ give us that $x = -1,\;y = 2,\;z = 1.$ }\\~\\
	\uncover<7->{Now, we calculate $f$ at the above point to get $6$ as an extremal value. To see that it is indeed the minimum and not the maximum, we substitute some other point like $(0, 3, 0)$ satisfying the constraints and compare. }\\~\\
\end{frame}
\end{document}