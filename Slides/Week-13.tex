...
\begin{frame}{Sheet 11}
	...
	\begin{align*}
		...
		\uncover<12->{&= \int_{-2}^{2} 2(\sqrt{4-x^2}) dx }\\
		\uncover<13->{&= 4\pi}
	\end{align*}
\end{frame}
\begin{frame}{Sheet 11}
...
\begin{frame}{Sheet 11}
	\uncover<1->{Consider the parameterisation of $C$ given by $(x(\theta),\;y(\theta)),$ where $x(\theta) := \rho(t)\cos(\theta(t)) \cos \theta$ and $y(\theta):=\rho(t)\sin(\theta(t))$ for $t \in[a, b].$ }\\~\\
	...
	\uncover<6->{$ = \displaystyle\oint_Cr^2d\theta.$ }\\~\\
	\uncover<7->{Note that $C$ was indeed positively oriented. }\hfill\uncover<8->{(How?) }
\end{frame}
\begin{frame}{Sheet 11}
...
\begin{frame}{Sheet 11}
	...
	\uncover<3->{Define the scalar fields $P := \frac{x}{x^2 + y^2}$ and $Q := -\frac{y}{x^2 + y^2}$ on $S.$ }\\
