...
\end{frame}
\begin{frame}{Sheet 12} 
	We shall consider the cylinder (and thus, the sphere) to have radius $1.$ Moreover, we shall choose our axes such that the center of the sphere is the origin and the axis of the cylinder is the $z-$axis.\\
...
\begin{frame}{Sheet 13} 
	...
	\begin{align*} 
		\uncover<2->{\mathbf{F} &= y \mathbf{i}+z \mathbf{j}+x \mathbf{k}, }\\
		\uncover<3->{\nabla\times\mathbf{F}&=-(\mathbf{i}+\mathbf{j}+\mathbf{k}). }
	\end{align*}
	\uncover<4->{Let $D := \{(x, y) \in \mathbb{R}^2 : x^2 + y^2 \le a^2\}.$ }\\~\\
	\uncover<5->{Let $f(x, y) := \dfrac{xy}{b}$ for $(x, y) \in D.$ }\\
\end{frame}
